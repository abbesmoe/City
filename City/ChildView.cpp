...
void CChildView::OnLButtonDown(UINT nFlags, CPoint point)
{
    ...
    if (mGrabbedItem != nullptr)
    {
        if (!mTransportActive)
        {
            ...
            CTransportTilesVisitor visitor(mCity);
...
/**
 *  Add a CTileTransporter tile to the drawing.
 * \param file The file for the building image
 */
void CChildView::AddTransporter(const std::wstring& file)
{
    auto tile = make_shared<CTileTransporter>(&mCity);
    tile->SetImage(file);
    tile->SetLocation(InitialX, InitialY);
    if (mFirstTransporter == true) {
        mFirstTransporter = false;
        tile->SetMinion(true);
    }
    mCity.Add(tile);
}
...
{
    AddTransporter(L"transporter.png");
}
...
{
    mTransportActive = !mTransportActive;
}


void CChildView::OnUpdateTransporterTransport(CCmdUI* pCmdUI)
{
    pCmdUI->SetCheck(mTransportActive);
}
